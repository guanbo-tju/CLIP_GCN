    ...
    def run_once(self, opt, run_engine_opt, log_dir, prev_log_dir=None, fold_idx=0):
        ...
        for net_name, net_info in net_info_opt.items():
            ...
            if pretrained_path is not None:
                ...
                # 使用新的函数加载权重  
                missing_keys, unexpected_keys = load_partial_state_dict(net_desc, net_state_dict)  
                ...
                # 添加更多详细信息  
                print(f"Successfully loaded {len(net_state_dict) - len(missing_keys)} layers.")  
                print(f"Skipped {len(missing_keys)} layers due to mismatch.")  
                if missing_keys:  
                    print("Skipped layers:", missing_keys)  
                #-----------------------------------------------------------------------------------------
